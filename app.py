...
def phishing_sms_prediction(sms_content):
    model = joblib.load("phishing_message_model.pkl")
    vectorizer = joblib.load("message_tfidf_vectorizer.pkl")
...
def store_in_database(content_type, content, content_class, response):
    document = {
        "content": content,
...
st.title("LLM-based Auto-Responder for Phishing Attacks")
...
if option == "URL":
    ...
    if st.button("Predict"):
        ...
        if prediction == 1:
            ...
            response = f"The website {url_content} seems like a phishing website. Kindly DO NOT visit the website and alert the IT/Security staff incase any vulnerability to data privacy and security is detected."
        else:
            st.success("The is a legitimate URL.")
            response = f"The website {url_content} is a legitimate website and there is no harm to data privacy and security while visiting this website."
        st.write(response)
